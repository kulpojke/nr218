...
Before downloading the data, prepare your project directory.   You will be working with 2 DEM datasets for this project, the OPR DEM, and the 1 arc second DEM.  Make a subdirectory within data for each
```
nr218/
└── lab_8
    ├── data
...
1. Open up the Symbology window for the OPR DEM layer (can elaborate here if needed).
2. Where it says `Render Type`, click the dropdown and select `Hillshade`.

This provides a quick, and not-great, hillshade rendering of the layer.  For a better hillshade, and/or to save the hillshade as a file, use __Raster__ > __Analysis__ > __Hillshade__.  Save the output to your `derivatives` directory as `hillshade_opr.tif`
...
This creates a vector layer of contour lines. Save it to your derivatives directory as contours.geojson


While these changes to symbology can be useful for quick visualizations of the landscape, if you were to send your DEM file to someone else, the adjustments wouldn't be reflected on their end. Luckily, QGIS also has processing tools that you can use to turn your hillshade and contour lines into separate layers that _can_ be shared.
...
2. Adjust the contour interval to the same value you used before (this tool does not specify an index interval).
