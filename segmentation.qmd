...
Download [Sentinel-2 sample data for a random AOI in Bangladesh](https://drive.google.com/file/d/1V7Mp5UjIZhh5Fsv6r7kFyLYCPKEybwxG/view?usp=sharing "Sentinel-2 Level-2A harmonized dataset for AOI in Bangladesh, west of Chittagong"){aria-label="Open Sentinel-2 dataset details for downloading imagery over a random AOI in Bangladesh"}
:::

::: {.fragment .absolute .fade-in-then-out data-fragment-index="1" style="max-width: 55%; font-size:0.4em;"}

+ Next we will create sample training points.
...
![](img/training_points_bangla.png){fig-alt="False color image with training points"}
...
Take some time to examine it using different visualizations.
...
Create a new point layer to use for training.

![](img/new_point_layer_for_training.png){width="45%" fig-alt="New Layer dialogue, be sure to click add to field list"}

Then add points and label the class ID appropriately
