...
## Some Common Vector File Formats {style="font-size:0.5em"}

| Format | File Extension | Stores Topology | Binary / Human Readable | Notes |
|--------|---------------|-----------------|------------------------|-------|
| Shapefile | .shp (+ .shx, .dbf, .prj) | No | Binary | Industry standard, but aging |
...
| TopoJSON | .topojson | Yes | Human Readable | Efficient for web, encodes topology |
| PostGIS | (database) | Yes (optional) | Binary | PostgreSQL spatial extension |
| File Geodatabase | .gdb | Yes (optional) | Binary | ESRI proprietary format |
