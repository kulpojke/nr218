...
## USGS Elevation data {style="font-size:0.7em"}

- LiDAR-derived products
    - Point clouds (`.laz`)
    - Project-level source data for high-resolution elevation products
- Raster elevation products
    - `1 m DEM`
        - Project-based bare-earth DEM, usually derived from lidar
    - `1/3 arc-second DEM` (`~10 m`)
        - Seamless 3DEP raster built from the best available elevation sources
    - `1 arc-second DEM` (`~30 m`)
    - `OPR = Original Product Resolution`
        - DEM distributed at the source project's original resolution and projection
        - Often the closest raster product to the original lidar-derived surface
...
## Aspect {.smaller}
[The direction slope is pointing in the X, y plane.]{style="font-size:0.7em"}

![](img/aspect.png){style="display:block; margin:0 auto; width:70%; max-height:90vh; object-fit:contain;"}

::: {.attribution style="font-size:0.4em"}
Image Source: [ESRI](https://pro.arcgis.com/en/pro-app/3.4/tool-reference/spatial-analyst/how-aspect-works.htm)
:::

## Finding DEMs {style="font-size:0.7em"}

There are several places to find USGS elevations data,

+ My favorite is the [USGS Lidar Explorer](https://www.usgs.gov/tools/lidarexplorer)
+ If you like comlexity [the national Map Viewer](https://apps.nationalmap.gov/viewer/)  is another option. 
+ Or [USGS Earth explorer](https://earthexplorer.usgs.gov/) where you can get all kinds of USGS data, but it is more time consuming to use.