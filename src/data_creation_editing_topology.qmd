...
Today we will build a small campus editing dataset:
...
1. Open QGIS.
2. Press `Ctrl+Shift+V` (`Cmd+Shift+V` on Mac) to open the Data Source Manager.
3. Under _Source Type_, select _File_.
4. Click the browse button and select `cal_poly_stream_network.gpkg`.
5. Click _Add_.
6. Add both layers:
   + `campus_boundary`
   + `stream_network`
7. Set the project CRS to `EPSG:26910` (NAD83 / UTM zone 10N).


::: {style="font-size:0.55em"}
[Oh! Speaking of NAD83: why official U.S. coordinates are changing](https://spacedaily.com/s-the-united-states-is-replacing-every-official-latitude-longitude-and-height-it-publishes-existing-coordinates-will-move-by-as-much-as-four-meters-and-the-million-survey-marks-in-the-ground-will-no/ "Article explaining that the National Geodetic Survey is replacing NAD83 and NAVD88, so published coordinates can shift even when physical locations do not move.")

::: {.attribution style="font-size:0.8em"}
Caption: Replacing a datum changes the reference frame used to describe locations, so latitude, longitude, and height values can change even when the ground stays put. Source: Space Daily, published September 10, 2026.
:::
:::
