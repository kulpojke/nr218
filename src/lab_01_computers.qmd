...
In this lab you will also be introduced to using the terminal. Having some familiarity with using terminals to interact with computers is useful because it forces you to understand some basic aspects of computer organization, and, if you continue after this class to pursue a career in GIS or any other computer intensive field, it is incredibly useful.  If you learn how to use the terminal well you will probably use it frequently.  That said, you will not have to be great at it for this class.  It will only be used for a couple of basic things, most importantly showing me how you are organizing your data.

For this lab you should create a text document (e.g. Word, LibreOffice, or Google docs, etc...) to which you will add the screenshot from Task 1 and answers from Task 2 (in steps 8, and 9). You will then upload the document to Canvas. You do not have to turn in anything for Task 3, having your OneDrive configured properly for the rest of the quarter will be helpful, so don't skip it!
...
A zip file is a type of compressed archive.  Zipping a folder full of sub-directories and files compresses the data (makes it take up less memory) while preserving the directory structure.  When you unzip, or _extract_,  the data you get back an uncompressed copy, which is just like the data that was zipped in the first place.  Zipped files are smaller (fewer bits), so they are good for transferring data, or long term storage when the data is not in use.  __Do not work directly with files compressed in a zip!__ .While QGIS and other applications _can_ read files from inside of a zip, it is a bad idea to do so.  There are many processing tools that cannot work with the compressed file.  Often you will get part of the way through a workflow and try some step of your analysis, only to get an error about unsupported file types.

You will now download some (not real) data, as a zip, and explore how it is arranged.

1. Download some made up files <a href="assets/toy_data.zip" download>here</a>. Most likely this will save the file in your Downloads folder.  
Open a terminal and check. Remember commands you learned in Task 1.  Hint: `Downloads/` is in your home directory 
<details>
<summary>Can't remember how, or having trouble? Click here.</summary>
<ul>
<li>To list the files in your Downloads directory use,   `ls ~/Downloads`  </li>
<li> Your Downloads directory in your home directory. The tilda, ~, is a shorthand notation for your home directory.</li>
<li>If there is a bunch of stuff in your Downloads folder you may want to include a portion of the filename to be more specific, `ls ~/Downloads/toy*`, where * is a wildcard, so _toy\*_  means anything beginning with _toy_.</li>
</ul>
</details>
While it is possible to work with data that is in your Download directory, it is a bad habit, and will likely lead to mayhem and frustration.  But for now, we will do just that, so unzip `toy_data.zip` right into the download folder (see @tip-zip for how to do this.).  
...
+ __Important:__ If you are not sure where your home folder is see @tip-home. Make sure you actually put it into your home directory. 
