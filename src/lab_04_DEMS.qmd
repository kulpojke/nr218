...
Make a new folder inside your `nr218` directory called `lab_4`. Inside that folder, make a `data` folder. All downloaded files will go in the `data` folder You should have a directory structure as shown below. 
...
_DEM Source (OPR)_ are DEMs distributed at the source project's original resolution and projection.  For Anacapa there should be 18 tiles under _DEM Source (OPR)_. If you hover over the titles of the slides, the extents of the tiles will be shown on the map, as can be seen in @fig-anacapa-tiles. 

Before downloading the data, prepare your project directory.   You will be working with 2 DEM datasets for this project, the OPR DEM, and the 1 arc second DEM.  Make a subdirectory within data for each, as shown below.
...
To download the OPR DEM you _could_ click the download button beside each tile and download it, it would be kind of a pain though.  If you were working on a larger area, clicking and downloading each file would not be practical.  It is useful to know how to
download the tiles in bulk using a download list.  The download list can be downloaded by pressing the _Download List_ button,  shown in @fig-download-list. The download list __is not the DEM data__ but just a list of URLs where the data is stored (in an Amazon Cloud Bucket).  In order to get the data, you must download the actual DEM tif files.  Downloading from a list is a common way of getting large tiled datasets. __For directions on downloading the data see__ @tip-wget. 
...
The following commands fetch the DEMs from the URLs in `downloadlist.txt`.  Note that they are assuming that `downloadlist.txt` is in the directory where you are running the command.  If it is not, you can adjust the path to `downloadlist.txt`(e.g. to `~/Downloads/downloadlist.txt` if it sits in your Downloads folder)  
...
+ The 1 arc second DEM is in EPSG:4269, a GCS CRS.  
...
At this point you are done using thea arc-second DEMs.  I suggest you remove them from your layer list to avoid confusion on subsequent parts of the lab.

## DEM derivative products (Using OPR DEM)

Next you will create a few different derivative products from your OPR DEMs. (you will not be using the arc second DEMs anymore for the lab.)  Some are useful as visualizations. Others are useful in analysis.
...
2. Make sure you are using your OPR DEM layer as the input.
3. Adjust the Z factor if you'd like. Keep the azimuth at its default for now.
4. Click on the 3 dots to the right of the `[Save to temporary file]` box and select `Save to file`. Navigate to your `data` folder and name the file something you can identify it by (i.e. include hillshade in the name). Before you hit `Save`, make sure the file format is `.tif`.
5. Leave all other parameters at their default and hit `Run`.

### Contour Lines

1. Now go to Raster > Extraction > Contour (or search Contour in the processing tools window).
2. Make sure you are using your OPR DEM layer as the input.
3. Adjust the contour interval to 10 meters (this tool does not specify an index interval).
4. Save the layer as a new file the same way you did above. Make sure that the file type is `.geojson`.
5. Leave all other parameters at their default and hit `Run`.


### Combining DEM Visualizations

1. Open up the Symbology window for your OPR DEM and change the Render Type to `Singleband pseudocolor`. Choose any graduated color ramp from the dropdown.
...
You can do this using a tool called _Sample Raster Values_.  Sample raster values samples the raster at a given point, and attaches that value as an attribute to the point. First, you will need some points, so download [this point dataset](https://raw.githubusercontent.com/kulpojke/nr218/main/assets/anacapa_sample_points.geojson) by right-clicking and using save link as.
