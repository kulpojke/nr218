...
1. Open a new project in QGIS, and save it as lab_14, in you lab 14 directory.
...
5. Save your model and name it `spr_model`. By default it will save to `models` folder that is specific to QGIS. Change it to your project folder.
6. Create a folder in you project called `tmp` for storing intermediate files.
...
│   ├── spr1/
│   │    ├── sprElevation_1.tif
│   │    ├── sprLanduse_1.geojson
│   │    ├── sprRoads_1.geojson
│   │    └── sprStreams_1.geojson
│   └── spr2/
│        ├── sprElevation_2.tif
│        ├── sprLanduse_2.geojson
│        ├── sprRoads_2.geojson
│        └── sprStreams_2.geojson
└── tmp/
```
...
![](img/add_raster_layer.png){width="40%" fig-alt="QGIS graphical modeler Inputs panel showing the Raster Layer option used to add a raster input." fig-cap="Add raster layer dialog in the QGIS graphical modeler."}

2. You should get a pop-up that looks like this:

![](img/layer_description.png){width="40%" fig-alt="QGIS graphical modeler Raster Layer Parameter Definition dialog for configuring a raster layer input." fig-cap="Raster layer parameter definition dialog in the QGIS graphical modeler."}
...
5. Add an input of type File/Folder.  This will be used to store intermediate files.
  a. Description: work_dir
  b. type: Folder
  c. for Default Value, use the browse button and find the tmp folder in you project directory

![](img/model_builder_work_dir.png)
...
1. The first thing we should do is ensure that all of our layers are in the same CRS.  The DEM is in EPSG:2227 (California state plane, zone 3), which will do.
  a. Set the project CRS to EPSG:2227.
  b. Switch from the ‘Inputs’ tab to the 'Toolbox' tab (depending on version may be ‘Algorithms’ tab), and search for ‘Reproject Layer’ (this should look very similar to the processing toolbox that we’re used to searching in!). 
  c. For each of our vector inputs, we want to reproject to EPSG:2227.
...
We’ll put our equation in the box labeled ‘Calculation in gdalnumeric syntax …’.  Remember we want a slope lower than 20 degrees.
