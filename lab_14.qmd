...
5. Save your model and name it `spr_model`. By default it will save to `models` folder that is specific to QGIS, but we should move it to our project folder.
...
1. The first thing we should do is ensure that all of our layers are in the same CRS. Switch from the ‘Inputs’ tab to the ‘Algorithms’ tab, and search for ‘Reproject Layer’ (this should look very similar to the processing toolbox that we’re used to searching in!). For each of our vector inputs, we want to reproject to EPSG:2227.
...
1. Remember, we want areas that are 1000 ft away from roads and within 500 ft of a stream. This means we’ll want to use the ‘Erase’ tool to remove our roads buffer, and then ‘Clip’ our remaining area to the streams buffer.
