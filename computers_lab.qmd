...
3. Open QGIS and start a blank project.
  ...
  + Press `Ctrl+Shift+V`, this is the keyboard shortcut to open a vector file (Do not worry if you do not yet know what I mean by vector file). Navigate to `~/Downloads/toy_data/WellNamedFiles` and select `Eaton_Perimeter_20250121.geojson`. You should now see a polygon in in the map (@fig-eaton)
...
8. Create a document in a text editor (e.g. notepad on Windows). Name it `gustav_adolf_lab1.txt` (use your own name) and save it in `~/nr218`.
9. View the contents of the directory:
  - In the terminal (while in `~/nr218`) type `tree` (on Windows `tree` omits files by default, so use `tree /F`)
